...
{
  const s = pres.addSlide();
  titleBar(s, "職員が使う画面　片手で終わる操作にする",
  ...
  const rowsStaff = [
    ["下端の固定タブに集約",
     ...
     "アイコンと文字を常に併記し、押す前に迷わせない"],
    ["希望休は1タップ、もう1回で取消",
     "以前は「区分を選ぶ→登録を押す」の2手。31日分を入れるには多すぎた"],
    ["31日×職員数の表は出さない",
  ...
  s.addText("同種の製品も職員側の機能は「シフトの確認」と「希望休の入力」に" +
    ...
    x: M, y: 6.55, w: W - M * 2, h: 0.4,
    fontFace: F.body, fontSize: 10, color: C.muted, margin: 0 });
  s.addNotes("30秒。左右の画面を指して『これが職員の全機能』と言う。" +
             "『確定前のシフトは見せない』は実務に合わせた判断として述べる。");
}

// ==================================================== 9 アカウントと監査ログ
{
  const s = pres.addSlide();
  titleBar(s, "運用に必要な機能　アカウントと監査ログ",
           "実地指導で問われるのは「基準を満たしていたか」と" +
           "「誰がいつ確定したか」の2点です");

  s.addText("アカウント管理　初回設定リンクを発行して本人が決める", {
    x: M, y: 1.6, w: 6.05, h: 0.34, fontFace: F.body, fontSize: 12,
    bold: true, color: C.deep, margin: 0 });
  shotTracked(s, "23_accounts.png", {
    x: M, y: 2.0, w: 6.05, h: 2.55,
    hint: "初回設定リンクの発行結果と一覧が写るように" });

  s.addText("監査ログ　追記専用。更新・削除・切り詰めを拒否", {
    x: M + 6.45, y: 1.6, w: W - M * 2 - 6.45, h: 0.34, fontFace: F.body,
    fontSize: 12, bold: true, color: C.deep, margin: 0 });
  shotTracked(s, "24_audit.png", {
    x: M + 6.45, y: 2.0, w: W - M * 2 - 6.45, h: 2.55,
    hint: "日時・実行者・操作・対象・内容・接続元が写るように" });

  const rows8 = [
    ["管理者が他人のパスワードを決めない",
     "初回設定と再設定はワンタイムリンク（単回・24時間）。" +
     "平文は保存せず SHA-256 のハッシュのみ。口頭やチャットに平文を残さない"],
    ["アカウントは削除せず無効化する",
     "監査ログが実行者を参照している。消すと「誰が操作したか」を辿れない。" +
     "無効化すると既存のログインもその場で切れる"],
    ["有効な管理者を0人にできない",
     "誰もアカウントを直せない状態を作らない。" +
     "自分自身の無効化と降格も拒否する"],
    ["追記専用をデータベースで担保する",
     "更新用のSQLを置かないだけでは手作業で消せる。" +
     "トリガで UPDATE / DELETE / TRUNCATE を拒否している"],
  ];
  let y8 = 4.75;
  rows8.forEach(([t, d]) => {
    s.addText(t, { x: M, y: y8, w: 4.3, h: 0.4, fontFace: F.body,
                   fontSize: 12, bold: true, color: C.dark, margin: 0,
                   valign: "middle" });
    s.addText(d, { x: M + 4.45, y: y8, w: W - M * 2 - 4.45, h: 0.4,
                   fontFace: F.body, fontSize: 11, color: C.muted, margin: 0,
                   valign: "middle" });
    y8 += 0.46;
  });
  s.addNotes("30秒。『シフトを作る機能だけでは実地指導に出せない』と言い、" +
             "監査ログがトリガで守られている点を強調する。");
}
